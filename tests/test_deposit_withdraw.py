...
def test_1st_deposit_and_last_withdraw(initial_prices, crypto_swap, coins, token, accounts):
    user = accounts[1]
...
def test_second_deposit(initial_prices, crypto_swap_with_deposit, token, coins, accounts):
    min_value = 10**16
    ...
    for x in range(MAX_SAMPLES):
        ...
        try:
            ...
            crypto_swap_with_deposit.add_liquidity(amounts, int(calculated * 0.999), sender=user)
...
def test_second_deposit_one(crypto_swap_with_deposit, token, coins, accounts):
    min_value = 10**16
...
def test_immediate_withdraw_one(crypto_swap_with_deposit, token, coins, accounts):
    user = accounts[1]
    ...
    for x in range(MAX_SAMPLES):
        for i in range(max_i+1):
            ...
            if token_amount >= token.totalSupply():
                ...
                try:
                    crypto_swap_with_deposit.remove_liquidity_one_coin(token_amount, i, int(0.999 * calculated), sender=user)
                except Exception:
